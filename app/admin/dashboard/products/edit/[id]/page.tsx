...
const parseSizeString = (sizeStr: string | null | undefined): { value: string; unit: string } => {
...
const EditProductPage = () => {
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        if (productData.image) {
         ...
         console.error("Detailed error fetching product:", { /* ... detailed error logging ... */ });
  ...
  const removeSizeField = (index: number) => {
    ...
    if (sizeToRemove && !sizeToRemove.id) {
        ...
        toast.info("Cannot remove saved size. Backend deletion feature not implemented.");
    }
  };
  ...
  const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setImage(null);
      setImagePreview(existingImageUrl ? `${process.env.NEXT_PUBLIC_API_URL}${existingImageUrl}` : null);
    }
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (product.isEnabled !== undefined) formData.append("isEnabled", String(product.isEnabled));
  ...
  return (
    <div className="container mx-auto py-10">
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardHeader>
            <CardTitle>Product Details {product.id ? `(ID: ${product.id})` : ''}</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-6">
            ...
            <div className="grid gap-4">
               ...
               {sizes.length > 0 ? (
                 sizes.map((sizeItem, index) => (
                   <div key={sizeItem.id || `new-${index}`} className="flex flex-wrap items-end gap-3 p-3 border rounded-md bg-gray-50/80 relative">
            ...
            <div className="grid gap-2">
              <Label htmlFor="image">Product Image (Leave blank to keep current)</Label>
