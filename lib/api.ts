  ...
  (error: AxiosError) => {
    ...
    if (error.response) {
      ...
      if (status === 401) {
        ...
        // Could redirect to login or clear invalid tokens
        if (typeof window !== 'undefined') {
...
const api = {
  ...
  auth: {
    ...
    signup: (data: { fullName: string; email: string; phoneNumber: string; password: string }) => 
  ...
  cart: {
    getGuestSession: () => 
      apiClient.get('/cart/guest-session'),
    ...
    getTotal: (params: string) => 
      apiClient.get(`/cart/total?${params}`),
    addItem: (data: any) => 
      apiClient.post('/cart/add', data),
    updateItem: (id: number, data: any) => 
      apiClient.patch(`/cart/${id}`, data),
  ...
  payment: {
    ...
    getGuestOrder: (orderId: string, email: string) =>
      apiClient.get(`/payment/snap/guest-order?orderId=${orderId}&email=${email}`),
  },
